...
@export var virtual_joystick: VirtualJoystickCF
...
func _on_mode_option_item_selected(index: int) -> void:
	var mode_key: StringName = VirtualJoystickCF.Modes.find_key(index)
	var mode: VirtualJoystickCF.Modes = VirtualJoystickCF.Modes[mode_key]
